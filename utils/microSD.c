...
bool readBlock( uint32_t blockaddr, uint8_t *buffer ) {
  ...
  // Check if peripheral is idle
  if(reply != 0x00 && reply != 0x01){
      return false;
  }

  // Read potential 0xFF's after R1 token and before data token
  do {
      reply = rxByteSSI3();
      } while( reply == 0xFF );

  // Check if data token is valid
  if (reply != 0xFE){
      return false;
  }
  ...
  // Read last 2 bytes of data corresponding to the CRC bytes
  rxByteSSI3();
  rxByteSSI3();
