...
void stringLCD( uint8_t *str ) {
  if( rightSideUp ) {
    ...
    // efficiency.  However, we have to more calculations up-front to
...
void initLCD( bool orientation ) {
  ...
  SYSCTL[SYSCTL_RCGCGPIO] |= SYSCTL_RCGCGPIO_PORTN | SYSCTL_RCGCGPIO_PORTM | SYSCTL_RCGCGPIO_PORTQ;
  SYSCTL[SYSCTL_RCGCGPIO] |= SYSCTL_RCGCGPIO_PORTN | SYSCTL_RCGCGPIO_PORTM | SYSCTL_RCGCGPIO_PORTQ;

  // Set all pins on Port M to inputs initially, then enable.
  GPIO_PORTM[GPIO_DIR] = 0;
  GPIO_PORTM[GPIO_AFSEL] = 0;
  GPIO_PORTM[GPIO_DEN] = 0xff;

  // Enable all control pins on Port N and make them outputs.
  GPIO_PORTN[GPIO_DIR] |= GPIO_PIN_5 | GPIO_PIN_4 | GPIO_PIN_3 | GPIO_PIN_2;
  GPIO_PORTN[GPIO_AFSEL] &= ~(GPIO_PIN_5 | GPIO_PIN_4 | GPIO_PIN_3 | GPIO_PIN_2);
  GPIO_PORTN[GPIO_PUR] &= ~(GPIO_PIN_5 | GPIO_PIN_4 | GPIO_PIN_3 | GPIO_PIN_2);
  GPIO_PORTN[GPIO_DEN] |= GPIO_PIN_5 | GPIO_PIN_4 | GPIO_PIN_3 | GPIO_PIN_2;
