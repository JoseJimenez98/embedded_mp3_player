...
// Refresh rate variable for calculations
static uint32_t refreshRate = 0;

// Volume control setting.
#define MAXVOLUME 32
uint8_t volume = (MAXVOLUME-2)/2;
...
void writeDACs( void ) {
  ...
  // Check if bufferRefilled is true
  if(bufferRefilled){

      // Set bufferRefilled to false
      bufferRefilled = false;

      // Update dataLength
      dataLength = dataLength + dataAdded;
  }

  // Check if there is data to transmit
  if(dataLength > 0){

      // Check buffers to see if low
      if(dataLength <= PADDING){

          // Set bufferLow to true
          bufferLow = true;
      }


        uint32_t left = *lOutPtr++;
        uint32_t right = *rOutPtr++;

        // Check if refresh rate has counted 44100 samples to equal one sec
        if(refreshRate == (44100)){

            // Create sound wavelet
            uint8_t wavelet = right;

            // Send wavelwet to be displayed
            addWavelet(wavelet);

            // Update UI
            updateUI();

            // Set refreshRate back to 0
            refreshRate = 0;
        }else{
        ...
        txDataSSI1( 0, (left * (uint32_t)volume) / 32);
        txDataSSI1( 1, (right * (uint32_t)volume) / 32);
        dataLength -= 1;

        // Loop lOutPtr and rOutPtr to beginning if they reach the end of the buffers
...
void upVolume( void ) {
    ...
    // Check if volume is at max volume (one less than max volume to avoid distortion)
    if(volume < (MAXVOLUME-2)){

        // Increase volume by 3 units
        volume += 3;
...
void downVolume( void ) {
    ...
    // Check if volume is at min volume
        if(volume > 0){

            // Decrease volume by 3 units
            volume -= 3;
        }
}
